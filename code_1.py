...
class AuthenticationSystem:
    ...
    def register_user(self, user_id, username, password):
        ...
        self.users = pd.concat([self.users, pd.DataFrame({
            ...
            "username": [username], 
            "password": [password], 
            "failed_attempts_left": [3], # <-- it should be initilize from 0
            "is_locked": [False]
        })], ignore_index=True) # Add new user to DataFrame. <--- comment
        print(f"User {username} registered successfully.")

    # Never alter this login function
    def login(self, username, password):
        user_row = self.users[self.users['username'].str.lower() == username.lower()] # <--- lowecase aur upper case ka gadbadi
        if user_row.empty:
        ...
        if password == password: # <-- yahan gaalt h 
            user.increment_failed_attempts() #<-- is line ko else me hona chaiye after 
                                             #if case ho thik kiya ham tab aur case_sensitive banae ke baad
            user.reset_failed_attempts()
            self.update_user(user)
            print(f"User {username} logged in successfully.")
        else:
            user.reset_failed_attempts() # <--- is line ko bhi yahan se hate yenge
            self.update_user(user)
